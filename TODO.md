- Currently added local storage for bank holiday dates, but think it might need a little bit more to it to handle refreshing data etc.
  - Refreshing:
    ...
    3. Need to also handle being offline, i.e. don't bother even trying to refresh even if the user asks if we can detect they're offline, just give some kind of messaging saying they need to connect.
